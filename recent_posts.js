function getPosts(json) {
    ...
    for (var i = 0; i < Math.min(numberOfPosts, json.feed.entry.length); i++) {
        ...
        if (showThumbs == true) {
            ...
            if (thumbUrl.indexOf("static.flickr.com") != -1) {
                thumbUrl = thumbUrl.replace("_b_t.jpg", "_s.jpg");
                thumbUrl = thumbUrl.replace("_m_t.jpg", "_s.jpg");
                thumbUrl = thumbUrl.replace("_s_t.jpg", "_s.jpg");
        ...
        try {
            ...
            if (showSummary == true) {
                ...
                if (postContent.length < summaryLength) {
...
function createPostItem(title, summary, thumb, link, linkTarget) {
    var li = tag("li", {className: "recentpost"}, [
            tag("div", {className: "thumb"},
                tag("a", {href: link, target: linkTarget},
                    tag("img", {src: thumb})
                )
            ),
            tag("div",{className: "description"},
                tag("h1", {},
                    tag("a", {href: link, target: linkTarget}, title)
                ),
